...
function EditStaffPage() {
    ...
    const {name,oldUsername,oldBranch,oldAge , oldNIC} = location.state;

    const [branchList, setBranchList] = useState([]);
    const [fullName, setFullName] = useState(name);
    const [username, setUsername] = useState(oldUsername);
    const [selectedBranch, setSelectedBranch] = useState(oldBranch);
    const [age, setAge] = useState(oldAge);
    const [nic, setNIC] = useState(oldNIC);
  ...
  const handleFormSubmit = () => {
    ...
    setAuthToken(token);
    axios.put("http://localhost:5000/staff", {
      fullName: fullName,
      username: username,
      ...
      <div className="flex flex-col items-center justify-center mt-5 bg p-3 ">
        <div className="w-[500px] flex flex-col items-center backdrop-blur-xl shadow-2xl rounded-3xl ">
          <div className="h-[60px] flex flex-row items-center text-[#ed2647] text-3xl font-semibold">
            <FaUsers className="mx-2" /> Edit Staff
          </div>
